...
//  Serpe's own shape and it is not MelGen's or ProgGenie's; what is shared is
...
struct SerpeMainView: View {
    var parameterTree: ObservableAUParameterGroup
    weak var audioUnit: SerpeAudioUnit?

    @State private var state = SerpeState()
    ...
    private var sound: some View {
        VStack(alignment: .leading, spacing: MelGenMetrics.space2) {
            ...
            ChipPicker(options: SerpeState.Voicing.allCases.map { ($0, $0.label) },
