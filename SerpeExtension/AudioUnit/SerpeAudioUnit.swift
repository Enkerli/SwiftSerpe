//
//  SerpeAudioUnit.swift
//  SerpeExtension
//
//  Serpe's half of the audio unit: the session, and what the kernel plays.
//
...
public final class SerpeAudioUnit: PluginAudioUnit, @unchecked Sendable {

    private let stateLock = NSLock()
    private var _state = SerpeState()
    ...
    func update(state newState: SerpeState, reloadKernel: Bool = true) {
        stateLock.withLock { _state = newState }
        if reloadKernel { loadIntoKernel(newState) }
    }

    private func loadIntoKernel(_ state: SerpeState) {
    ...
    private static let stateKey = "Serpe.sessionState"
    ...
    public override var fullState: [String: Any]? {
        ...
        set {
            ...
            guard let data = newValue?[Self.stateKey] as? Data,
                  let restored = try? JSONDecoder().decode(SerpeState.self, from: data) else {
                return
            }
            state = restored
        }
