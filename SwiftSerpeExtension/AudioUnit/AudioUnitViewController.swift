...
//  SwiftSerpeExtension
//
//  SwiftSerpe's principal class: the three things a plug-in tells the shell.
//
...
public final class AudioUnitViewController: PluginViewController {
    ...
    public override func makeAudioUnit(componentDescription: AudioComponentDescription) throws -> PluginAudioUnit {
        try SwiftSerpeAudioUnit(componentDescription: componentDescription, options: [])
    }

    public override var parameterTreeSpec: ParameterTreeSpec { SwiftSerpeParameterSpecs }

    public override func makeRootView(parameterTree: ObservableAUParameterGroup,
                                      audioUnit: PluginAudioUnit) -> AnyView {
        AnyView(SwiftSerpeMainView(parameterTree: parameterTree,
                              audioUnit: audioUnit as? SwiftSerpeAudioUnit))
    }
}
